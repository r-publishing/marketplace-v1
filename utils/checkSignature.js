...
const checkSignature = async (folder, s) => {
  const did = new DID({
    resolver: { ...(await getRchainResolver), ...KeyResolver.getResolver() },
  });
  ...
  const included = Object.fromEntries(
    Object.entries(doc2.signatures).filter(
      ([key, value]) => parseInt(key) < parseInt(s)
    )
  );
  ...
  try {
    const signer = verifyJWS(
      signature.protected + "." + payloadCid + "." + signature.signature,
      res.publicKey
    );
